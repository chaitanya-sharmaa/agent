...
OUTPUT ONLY THE JSON ABOVE.'''
    ...
    async def _run_comprehensive_workflow(self):
        ...
        peer_items = cluster_resources.get("peerauthentication", {}).get("items", []) if 'cluster_resources' in locals() else []
        if not peer_items:
        ...
        authz_items = cluster_resources.get("authorizationpolicy", {}).get("items", []) if 'cluster_resources' in locals() else []
        if not authz_items:
        ...
        crb_items = cluster_resources.get("clusterrolebindings", {}).get("items", []) if 'cluster_resources' in locals() else []
        ...
        security_findings["suggestions"].append("• Enable Pod Security Standards (PSS/PSA) enforcing baseline/restricted")
        security_findings["suggestions"].append("• Implement admission controls (OPA/Gatekeeper/Kyverno)")
